...
[role="lead"]
在 NetApp Disaster Recovery 中配置复制计划后，您可以执行故障转移、测试故障转移、刷新资源、迁移以及管理快照和复制计划。除维护外，主要执行的操作是在发生故障时进行故障转移。
...
. 选择要使用的快照。要创建生产环境的最新副本，请选择*立即拍摄快照*。要使用以前捕获的快照，请选择*选择*，然后选择现有快照。
...
每当您对虚拟基础架构进行更改（例如添加或删除虚拟机、添加或删除数据存储区或在数据存储区之间移动虚拟机）时，您都需要在灾难恢复中刷新受影响的 vCenter 集群。默认情况下，灾难恢复每 24 小时自动刷新一次，但手动刷新可确保最新的虚拟基础架构信息可用，并在灾难恢复保护中得到考虑。
...
除了使用 NetApp Disaster Recovery 来管理灾难时的复制和故障转移之外，它还可以支持从源站点到目标站点的一次性 VM 迁移。在应对极端天气或政治动荡等灾难性事件时，您可能会执行一次性迁移，以此作为迁移到云端或灾难避免程序的一部分。
...
由于 NetApp Disaster Recovery 协调 ONTAP 卷快照，因此 ONTAP 存储管理员可以在服务不知情的情况下，使用 ONTAP System Manager、ONTAP CLI 或 ONTAP REST API 直接删除快照。Disaster Recovery 每 24 小时自动删除源集群上所有不在目标集群上的快照。您可以根据需要手动删除快照。此功能可确保所有站点的快照保持一致。
