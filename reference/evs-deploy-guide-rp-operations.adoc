...
summary: 使用 Amazon EVS 和Amazon FSx for NetApp ONTAP介绍NetApp灾难恢复。使用NetApp Disaster Recovery 与 Amazon EVS 和Amazon FSx for NetApp ONTAP执行故障转移、测试故障转移、刷新资源、迁移、立即拍摄快照、禁用或启用复制计划、清理旧快照、协调快照、删除复制计划和编辑计划。 
---
= 使用NetApp灾难恢复执行复制计划操作
:allow-uri-read: 


[role="lead"]
使用NetApp Disaster Recovery 与 Amazon EVS 和Amazon FSx for NetApp ONTAP执行以下操作：故障转移、测试故障转移、刷新资源、迁移、立即拍摄快照、禁用/启用复制计划、清理旧快照、协调快照、删除复制计划和编辑计划。
...
. 选择要用于恢复的备份。
. （可选）选择是否希望NetApp Disaster Recovery 强制执行故障转移过程，而不管复制计划的状态如何。这只能作为最后的手段。
. （可选）选择是否希望NetApp Disaster Recovery 在生产站点恢复后自动创建反向保护关系。
...
.步骤
. 选择“操作”选项image:../use/icon-horizontal-dots.png["NetApp灾难恢复服务中的操作菜单图标"]复制计划旁边。
. 从复制计划的操作菜单中选择*运行合规性检查*选项：
+
image:evs-rp-menu-compliance-check.png["运行合规性检查菜单选项"]

. 要更改NetApp Disaster Recovery 自动运行合规性检查的频率，请从复制计划的操作菜单中选择“编辑计划”选项。
...
每次对虚拟基础架构进行更改（例如添加或删除虚拟机、添加或删除数据存储区或在数据存储区之间移动虚拟机）时，都需要在NetApp灾难恢复服务中对受影响的 vCenter 集群执行刷新。该服务默认每 24 小时自动执行一次此操作，但手动刷新可确保最新的虚拟基础设施信息可用并考虑到 DR 保护。
...
虽然NetApp灾难恢复主要用于灾难恢复用例，但它也可以将一组虚拟机从源站点一次性移动到目标站点。这可能是为了协调迁移到云项目，也可能是为了避免灾难——例如恶劣天气、政治冲突或其他潜在的暂时灾难性事件。
...
您可以随时立即拍摄复制计划的快照。此快照包含在复制计划的快照保留计数设置的NetApp灾难恢复考虑因素中。
