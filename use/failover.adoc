...
在进行实时故障转移之前，您可以执行测试以检查您的准备情况。工作负载在测试过程中保持在线状态。
...
+
然后输入"Test failover"并选择 *Test fail over*。
...
IMPORTANT: 如果复制计划包含空数据存储区，则必须首先从计划中删除空数据存储区，或使用 **强制故障转移** 选项，该选项将跳过空数据存储区。
...
. 如果您希望发生故障转移，即使检测到通常会阻止故障转移发生的错误，请选择*强制故障转移*。
