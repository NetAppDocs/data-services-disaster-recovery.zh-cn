...
[role="lead"]
添加资源站点后，您可以创建资源组，将虚拟机、数据存储区或 Kubernetes 集群组作为一个单元进行分组。资源组使您能够将一组从属资源组织成满足您要求的逻辑组。例如，您可以对与一个应用程序关联的虚拟机进行分组，也可以对具有类似层的应用程序进行分组。作为另一个示例，组可能包含可在恢复时运行的延迟引导顺序。
...
+
image:kubernetes-resource-group.png["添加 Kubernetes 资源组"]
