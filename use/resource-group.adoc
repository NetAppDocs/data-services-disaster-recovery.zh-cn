...
summary: 资源组使您能够将一组从属资源组织成满足您要求的逻辑组。 
---
...
[role="lead"]
向站点添加资源后，您可以创建资源组来将虚拟机、数据存储区或 Kubernetes 集群组织为一个单元。资源组使您能够将一组从属资源组织成满足您要求的逻辑组。例如，您可以对与一个应用程序关联的虚拟机进行分组，也可以对具有类似层的应用程序进行分组。作为另一个示例，组可能包含可在恢复时运行的延迟引导顺序。
...
TIP: 每个资源组可以包括一个或多个虚拟机、数据存储或 Kubernetes 集群。虚拟机将根据您将它们包含在复制计划中的顺序启动。您可以通过向上或向下拖动资源组列表中的虚拟机或数据存储来更改顺序。
...
. 对*资源类型*进行分类。默认情况下，选择"包括所有"，表示多个资源类型
