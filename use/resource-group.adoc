...
[role="lead"]
添加 vCenter 站点后，您可以创建资源组，将 VM 组或数据存储库组作为一个单元。资源组使您能够将一组相关 VM 组织成满足您要求的逻辑组。例如，您可以对与一个应用程序关联的 VM 进行分组，也可以对具有类似层的应用程序进行分组。作为另一个示例，组可能包含可在恢复时运行的延迟启动顺序。
