...
[role="lead"]
要使用 NetApp Disaster Recovery，请在 NetApp Console 和支持的灾难恢复环境中完成几个设置步骤。
...
* 混合云灾难恢复，将本地 VMware 和 ONTAP 数据中心复制到基于 VMware Cloud on AWS 或 Elastic VMware Service (EVS) 和 Amazon FSx for NetApp ONTAP 的 AWS 灾难恢复基础架构。
* 私有云 DR 将本地 VMware 和ONTAP vCenter 复制到另一个本地 VMware 和ONTAP vCenter。
* 云 DR，将基于 VMware Cloud on AWS 或 EVS 的 AWS DR 基础架构复制到另一个使用 FSx for NetApp ONTAP 的基于 AWS 的 DR 基础架构。
* 混合云 DR，将本地 VMware 和 ONTAP 数据中心复制到基于 Google Cloud VMware Engine 和 Google Cloud NetApp Volumes 的 Google DR 基础架构。
...
生产和 DR 站点通过 AWS 兼容的安全连接进行连接。常见的连接类型包括安全 VPN（私有或 AWS 提供）、AWS Direct Connect 或其他已批准的互连方法。
...
*最佳实践*：对于 AWS DR 站点配置，NetApp 建议对 VMware Cloud 和 AWS FSx for ONTAP 实例使用单 AZ 部署。由于 AWS 用于灾难恢复，因此引入多个可用区没有优势。多 AZ 部署可能会增加成本和复杂性。
