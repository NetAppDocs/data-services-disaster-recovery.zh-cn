...
[role="lead"]
要使用 NetApp Disaster Recovery，请首先在中断之前准备好恢复环境，以便能够以最短的延迟配置和测试复制及故障转移。
...
* 混合云灾难恢复，将本地 VMware 加 ONTAP 数据中心复制到基于 VMware Cloud on AWS 或 Elastic VMware Service (EVS) 和 Amazon FSx for NetApp ONTAP 的 AWS 灾难恢复基础架构。
* 将本地 VMware 加 ONTAP vCenter 复制到另一个本地 VMware 加 ONTAP vCenter 的私有云灾难恢复。
* 使用 FSx for NetApp ONTAP，将基于 VMware Cloud on AWS 或 EVS 的 AWS 灾难恢复基础架构复制到另一个基于 AWS 的灾难恢复基础架构的云灾难恢复解决方案。
* 混合云灾难恢复，将本地 VMware 加 ONTAP 数据中心复制到基于 Google Cloud VMware Engine 和 Google Cloud NetApp Volumes 的 Google 灾难恢复基础设施。
...
此方法包含一个本地生产 vCenter 基础架构，该基础架构使用通过 NFS 协议托管在 ONTAP FlexVol 卷上的数据存储。灾难恢复站点包含一个或多个 VMware Cloud SDDC 实例，这些实例使用通过 NFS 协议托管在一个或多个 FSx for ONTAP 实例提供的 FlexVol 卷上的数据存储。

生产和灾难恢复站点通过与 AWS 兼容的安全连接进行连接。常见的连接类型包括安全 VPN （私有或 AWS 提供）、AWS Direct Connect 或其他经批准的互连方法。
...
AWS 支持在一个区域内的一个或多个可用区 (AZ) 中部署解决方案。NetApp Disaster Recovery 使用两种 AWS 托管服务：VMware Cloud on AWS 和 Amazon FSx for NetApp ONTAP。

* *VMware Cloud for AWS*：支持在单 AZ 或双 AZ 扩展集群 SDDC 环境中进行部署。NetApp Disaster Recovery 仅支持 VMware Cloud on AWS 的单 AZ SDDC 部署。
* *AWS FSx for NetApp ONTAP*：在多 AZ 配置中部署 Amazon FSx for NetApp ONTAP 时，每个卷由一个 FSx for ONTAP 文件系统拥有，数据将镜像到第二个文件系统。FSx for ONTAP 可以部署在单 AZ 或多 AZ 配置中。NetApp Disaster Recovery 支持这两种部署模式。


*最佳实践*：对于 AWS 灾难恢复站点配置，NetApp 建议对 VMware Cloud 和 AWS FSx for ONTAP 实例使用单 AZ 部署。由于 AWS 用于灾难恢复，因此引入多个可用区没有优势。多 AZ 部署可能会增加成本和复杂性。
...
* *AWS Direct Connect*：这是 AWS 云互连，与您的私有数据中心位于同一地理区域，由 AWS 合作伙伴提供。此解决方案可在您的本地数据中心和 AWS 云之间提供安全的私有连接，而无需公共互联网连接。这是 AWS 提供的最直接、最有效的连接方法。
...
* *AWS Site-to-Site VPN*：此虚拟专用网络连接可通过公共互联网服务提供商提供安全连接。VPN 对往返 AWS 云的所有数据进行加密和解密。VPN 可以是基于软件的，也可以是基于硬件的。对于企业应用程序，公共互联网服务提供商（ISP）应提供服务质量保证，以确保为灾难恢复复制提供足够的带宽和延迟。


*最佳实践*：对于 AWS 灾难恢复站点配置，NetApp 建议使用 AWS Direct Connect。此解决方案为企业级应用程序提供最高的性能和安全性。如果不可用，则应使用高性能公共 ISP 连接以及 VPN。确保 ISP 提供商业 QoS 服务级别，以确保充足的网络性能。
...
*最佳实践*：对于使用 VMware Cloud 和单个 FSx for ONTAP VPC 的灾难恢复解决方案，NetApp 建议使用 VPC 对等。如果部署了多个 FSx for ONTAP VPC，则建议使用 AWS Transit Gateway 来减少多个 VPC 对等连接的管理开销。
...
** 按照以下步骤操作 https://docs.netapp.com/us-en/netapp-solutions/ehc/aws/aws-native-overview.html["技术报告 4938：使用 VMware Cloud on AWS 将Amazon FSx ONTAP挂载为 NFS 数据存储"^]和 https://docs.netapp.com/us-en/storage-management-fsx-ontap/start/task-getting-started-fsx.html["Amazon FSx for NetApp ONTAP快速入门"]为ONTAP配置和部署 FSx。


* 将Amazon FSx for ONTAP添加到系统，并为 FSx for ONTAP添加 AWS 凭证。
* 在 AWS FSx for ONTAP 实例中创建或验证您的目标 ONTAP SVM。
...
灾难恢复支持使用以太网和 TCP/IP 的站点到站点复制。确保有足够的带宽来支持生产站点虚拟机上的数据更改率，以便在恢复点目标 （RPO） 时间范围内将所有更改复制到灾难恢复站点。
...
NetApp Disaster Recovery 代理实现需要控制台代理与使用 HTTPS 协议的任何 vCenter 服务器和 ONTAP 阵列之间进行端口 443 通信。控制台代理中的 NetApp Disaster Recovery 代理在执行操作时直接与 VMware vSphere、vCenter 服务器或 ONTAP 通信。
