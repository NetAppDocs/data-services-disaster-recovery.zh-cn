...
[role="lead"]
云灾难恢复是保护工作负载免受站点中断和数据损坏事件影响的一种弹性和经济高效的方式。借助 NetApp Disaster Recovery，您可以将运行 ONTAP 存储的本地 VMware VM 或数据存储工作负载复制到使用 NetApp 云存储的公共云中的 VMware 软件定义数据中心，或复制到另一个将 ONTAP 存储作为灾难恢复站点的本地 VMware 环境。您还可以使用 Disaster Recovery 将 VM 工作负载从一个站点迁移到另一个站点。

NetApp Disaster Recovery 是一项基于云的灾难恢复服务，可自动化灾难恢复工作流。使用 NetApp Disaster Recovery，您可以保护本地 Kubernetes 集群、基于 NFS 的工作负载，以及运行在 NetApp 存储上的 iSCSI 和 FC 的 VMware vSphere 虚拟机文件系统（VMFS）数据存储，保护对象包括以下之一：
...
Disaster Recovery 使用以下组件来提供灾难恢复工作负载：
