...
summary: 云灾难恢复是保护工作负载免受站点中断和数据损坏事件影响的一种弹性和经济高效的方式。使用 NetApp Disaster Recovery，您可以将运行 ONTAP 存储的本地 VMware 工作负载复制到使用 NetApp 云存储的公共云中的 VMware 软件定义数据中心，或复制到将 ONTAP 存储作为灾难恢复站点的另一个本地 VMware 环境。 
---
...
[role="lead"]
云灾难恢复是保护工作负载免受站点中断和数据损坏事件影响的一种弹性和经济高效的方式。NetApp Disaster Recovery 是一种基于云的灾难恢复服务，可自动执行灾难恢复工作流程。使用 NetApp Disaster Recovery，您可以保护本地 Kubernetes 集群、基于 NFS 的工作负载，以及运行在 NetApp 存储上的 iSCSI 和 FC 的 VMware vSphere 虚拟机文件系统（VMFS）数据存储，保护对象包括以下之一：

* Amazon Elastic VMware Service (EVS) with Amazon FSx for NetApp ONTAP。有关详细信息，请参阅 link:../reference/evs-deploy-guide-introduction.html["使用 Amazon Elastic VMware Service 和 Amazon FSx for NetApp ONTAP 的 NetApp Disaster Recovery 简介"]。
* AWS 上的 VMware Cloud (VMC) 以及适用Amazon FSx for NetApp ONTAP
* Azure VMware Solution 与 NetApp Cloud Volumes ONTAP (iSCSI)（私有预览）
* Google Cloud VMware Engine (GCVE) 与Google Cloud NetApp Volumes
* 另一个具有 ONTAP 存储的基于 NFS 和/或 VMFS (iSCSI/FC) 的本地 VMware 环境
...
image:../use/evs-console-architecture.png["显示 NetApp Disaster Recovery 组件的图表"]
...
开始使用 NetApp Console 无需许可证或订阅，仅在需要在云中部署 Console 代理以确保连接到存储系统或 NetApp 数据服务时才会产生费用。但是，从 Console 访问的一些 NetApp 数据服务是许可或基于订阅的。

详细了解 https://docs.netapp.com/us-en/console-setup-admin/concept-overview.html["NetApp Console"]。



[[benefits-of-using-netapp-disaster-recovery]]
== 使用 NetApp Disaster Recovery 的优势
...
[[capabilities-of-netapp-disaster-recovery]]
== NetApp Disaster Recovery 的功能
...
* 执行故障转移测试。执行此操作时，该服务将创建临时虚拟机。Disaster Recovery 将从所选快照创建一个新的 FlexClone 卷，并将由该 FlexClone 卷支持的临时数据存储区映射到 ESXi 主机。此过程不会消耗本地 ONTAP 存储或 AWS 中 FSx for NetApp ONTAP 存储的额外物理容量。原始源卷不会被修改，即使在灾难恢复期间，副本作业也可以继续。
...
image:../use/evs-use-cases.png["显示 NetApp Disaster Recovery 用例的图表"]


NOTE: 您必须在 vSphere 服务器中，在 NetApp Disaster Recovery 之外配置 vSphere 服务器。
...
* 注册即可获得 30 天免费试用。
* 通过 Amazon Web Services (AWS) Marketplace、Azure Marketplace 或 Google Cloud Marketplace 购买按需付费 (PAYGO) 订阅。此许可证允许您购买固定受保护容量许可证，而无需任何长期承诺。
...
Disaster Recovery 的许可基于受保护 ONTAP 卷上托管的数据量。该服务通过将受保护的虚拟机映射到其 vCenter 数据存储区来确定哪些卷应考虑用于许可目的。每个数据存储区都托管在 ONTAP 卷或 LUN 上。ONTAP 为该卷或 LUN 报告的已用容量用于许可确定。

受保护的卷可以托管许多虚拟机。有些可能不属于NetApp Disaster Recovery资源组。无论如何，该卷或 LUN 上所有虚拟机消耗的存储都将用于满足许可证最大容量要求。


NOTE: NetApp Disaster Recovery 费用基于源站点上数据存储的已用容量（当至少有一个虚拟机或 Kubernetes 资源具有复制计划时）。故障转移数据存储的容量不包含在容量限额中。对于 BYOL，如果数据超出允许的容量，则在您获得额外的容量许可证或在 NetApp Console 中升级许可证之前，服务中的操作将受到限制。
...
您可以通过 30 天免费试用来试用 NetApp Disaster Recovery。

要在 30 天试用期后继续，您需要从云提供商处获得现收现付 (PAYGO) 订阅，或从 NetApp 购买 BYOL 许可证。
...
image:dr-architecture-diagram-70-2.png["显示 NetApp Disaster Recovery 服务基础架构的架构图"]
...
* AWS 上的 VMware Cloud (VMC) 以及适用Amazon FSx for NetApp ONTAP
* 另一个具有 ONTAP 存储的基于 NFS 的本地 VMware 环境，或本地 FC/iSCSI VMFS 环境
