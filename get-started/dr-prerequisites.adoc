...
* 故障转移仅支持使用最新快照版本。如有必要，您可以在创建快照期间创建新快照（即必须禁用选择性快照选项）。
* 故障转移后无法创建新的快照。
* 故障转移后无法保留和协调快照。
...
* 所需的控制台代理类型：
+
** *本地到本地灾难恢复*：在灾难恢复站点安装控制台本地代理。使用此方法，主站点的故障不会阻止服务在 DR 站点重新启动虚拟资源。参考 https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["在本地安装并设置控制台代理"^]。
