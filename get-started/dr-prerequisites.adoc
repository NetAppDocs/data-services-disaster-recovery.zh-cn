...
summary: 在使用NetApp Disaster Recovery之前，您应该确保您的环境满足ONTAP存储、VMware vCenter 集群和NetApp Console要求。 
---
...
[role="lead"]
在使用NetApp Disaster Recovery之前，您应该确保您的环境满足ONTAP存储、VMware vCenter 集群和NetApp Console要求。
...
+
** 您的 VPC 中必须存在用于托管 VMware DR 数据存储区的Amazon FSx for NetApp ONTAP实例。请参阅Amazon FSx for ONTAP文档 https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/getting-started-step1.html["如何开始"^]。
...
* 必须将Amazon FSx for ONTAP和 AWS 凭证添加到NetApp Console项目中的系统，用于管理NetApp Disaster Recovery。
...
** 已安装的控制台代理必须能够访问NetApp Disaster Recovery将管理的任何 VMware 集群。
...
* 对于运行 Microsoft SQL Server 或 Oracle Database 或两者的 Windows VM，数据库应启用其 VSS Writer。
