...
[role="lead"]
在安装之前验证存储、vCenter 和控制台的先决条件，以便工作负载为复制和恢复做好准备。
...
当您将 NetApp Disaster Recovery 与 Google Cloud VMware Engine 和 Google Cloud NetApp Volumes 结合使用时，请确保配置正确的权限并查看以下注意事项。

* 请联系 Google SRE 团队，将以下内容加入允许列表：
+
...
这些先决条件适用于 ONTAP 和 Amazon FSx for NetApp ONTAP 实例。

* 源集群和目标集群必须具有对等关系。
* 承载灾难恢复卷的 SVM 必须存在于目标集群上。
* 源 SVM 和目标 SVM 必须具有对等关系。
* link:task-kubernetes-requirements.html["查看 Kubernetes 集群的附加要求"].
...
如果您尚未完成此操作， https://docs.netapp.com/us-en/console-setup-admin/task-sign-up-saas.html["注册 NetApp Console 并创建组织"^]。
...
TIP: 要将灾难恢复与 Amazon Elastic VMware Service (EVS) 一起使用，请参见 link:../reference/evs-deploy-guide-config-prereqs.html["其他先决条件和配置指南"^]。
...
IMPORTANT: 根据您的部署类型，查看 link:reference-vmware-prerequisites.html["VMware 集群先决条件"] 和 link:task-kubernetes-requirements.html["Kubernetes 集群先决条件"]。
